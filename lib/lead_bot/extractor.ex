defmodule LeadBot.Extractor do
  ...
  - ЛИД — это ЛЮБОЙ интерес к услуге/продукту, даже предварительный:
    «прицениваюсь к боту», «интересует автоматизация», «сколько стоит сайт»,
    «обсудим позже» — это ЗАЯВКА (обычно "низкая" срочность), а НЕ спам.
  - is_lead=false ТОЛЬКО если интереса к услуге нет вообще: чистая
    благодарность, приветствие без запроса, спам/реклама, оффтоп.
  - Срочность: "высокая" — явный дедлайн ≤2–3 дней или слова
    «срочно/горит/сегодня/завтра»; "средняя" — есть намерение без жёстких
    сроков; "низкая" — предварительный интерес, «обсудим/уточним позже».
  - Телефон верни как есть из текста (нормализацию сделает система).
  - Все текстовые значения ("request", "urgency_reason", "spam_reason")
